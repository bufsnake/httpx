						...
						for _, node_res := range node_ress {
							...
							for keys_, vals_ := range vals {
								...
								case "properties":
									...
									if exception != nil {
										w.log.Error(name, exception.Error())
										continue
									}
	...
	return chromedp.ActionFunc(func(ctx context.Context) error {
		...
		for name, value := range schemas {
			...
			case map[string]string:
				for key, val := range v {
					for i := 0; i < len(attributes); i++ {
						if utils.IsArrExist(attributes[i], key) && utils.IsArrExist(attributes[i], val) {
							w.setFinger(name, value, 100, "")
						}
					}
				}
			...
			case map[string][]string:
				for key, val := range v {
					for i := 0; i < len(attributes); i++ {
						if utils.IsArrExist(attributes[i], key) {
							for j := 0; j < len(val); j++ {
								if utils.IsArrExist(attributes[i], val[j]) {
									w.setFinger(name, value, 100, "")
								}
							}
						}
